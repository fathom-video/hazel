...
module.exports = ({ cache, config }) => {
  ...
  exports.downloadFile = async (req, res) => {
    ...
    const latest = await loadCache()
    const possibleAssets = Object.values(latest.platforms).filter((o) => o.name == filename)
    if (possibleAssets.length === 0) {
      send(res, 404, 'This download is not available')
      return
    }
    const selectedAsset = possibleAssets[0]

    if (shouldProxyPrivateDownload) {
      proxyPrivateDownload(selectedAsset, req, res)
    } else {
      res.setHeader('Location', selectedAsset.url)
      send(res, 302)
    }
  }

  exports.updateDarwinWithArch = async (req, res) => {
    const { arch, version } = req.params

    res.setHeader('Location', (arch === 'arm64' ? `/update/darwin_${arch}/${version}` : `/update/darwin/${version}`))
    send(res, 302)
  }
