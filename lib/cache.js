...
module.exports = class Cache {
  ...
  async cacheReleaseList(asset) {
    const { token, url: server_url } = this.config
    const headers = { Accept: 'application/octet-stream' }
    const shouldProxyPrivateDownload =
      token && typeof token === 'string' && token.length > 0
    const { browser_download_url, url: api_url } = asset

    const download_url = (shouldProxyPrivateDownload ? api_url : browser_download_url)
    ...
    const { body } = await retry(
      async (bail) => {
        ...
        if (response.status !== 200) {
          bail(new Error(`Tried to cache RELEASES, but failed fetching ${download_url}, status ${response.status}`))
        }
    ...
    for (let i = 0; i < matches.length; i += 1) {
      if (shouldProxyPrivateDownload) {
        content = content.replace(matches[i], `${server_url}/download/file/${matches[i]}`)
      } else {
  ...
  async refreshCache() {
    ...
    while (!release) {
      ...
      const relations = parseLinkHeaders(response.headers.get('link'))
      if (relations && relations.next && relations.next.url) {
        url = relations.next.url
      } else {
        break
      }
    }
